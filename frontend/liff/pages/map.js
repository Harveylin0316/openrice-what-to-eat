...
let mrtLayer = null;            // 捷運站錨點層（r60：真實站點座標，z≥14 起一路顯示不消失）
...
function dealBadgesHtml(pin, { compact = false } = {}) {
    let html = '';
    ...
    // compact（迷你卡）：已有加碼 badge 時省略回饋 badge——明細行有完整說明，badge 疊兩行太佔畫面
    if (pin.b && !(compact && (pin.hm || pin.ho))) html += '<span class="map-badge map-badge--cashback">出席回饋 NT$3/人</span>';
    return html;
}
...
function walkLabel(lat, lng) {
    ...
    return `走路 ${Math.max(1, Math.round(m * 1.25 / 80))} 分`;
}
...
// r40 灰點降噪：中低 zoom 灰點退成半透明底紋，彩色優惠釘才是主角（Google 式層級——
// 次要 POI 淡、重點 POI 飽和）；z≥17 街道層級恢復實心（那時灰點就是逛街的內容本身）。
// 命中不受影響：canvas tolerance 12px + nearestDotAt 消歧照舊。
const DOT_MUTED = { opacity: 0.6, fillOpacity: 0.55 };
const DOT_SOLID = { opacity: 1, fillOpacity: 0.9 };
...
// Google 一載入就有「行天宮、麥當勞」等地名當空間錨點，我們的底圖磚在中低 zoom
// 幾乎沒字 → 用戶不知道自己在看哪。兩個來源：
//   1. 搜尋索引的商圈/站名（places, t=landmark）＋ landmarks.json 的知名地標(spots)
...
// 府中 vs 府中商圈）放寬到 ≈900m 也只留一個——兩張幾乎同名的標籤比沒有標籤更混亂。
function clashesKeptAnchor(p) {
    return landmarkKept.some(k =>
        (Math.abs(k.lat - p.lat) < 0.0027 && Math.abs(k.lng - p.lng) < 0.003) ||
        ((k.n.includes(p.n) || p.n.includes(k.n)) &&
            Math.abs(k.lat - p.lat) < 0.008 && Math.abs(k.lng - p.lng) < 0.009));
}

function buildLandmarkLayer(L) {
    if (!map) return;
    const cands = allPlaces
        .filter(p => p.t === 'landmark' && Number.isFinite(p.lat) && Number.isFinite(p.lng))
        .sort((a, b) => (b.c || 0) - (a.c || 0));
    landmarkKept = [];
    for (const p of cands) {
        if (landmarkKept.length >= 28) break; // 商圈錨點是配角，鋪滿反而變雜訊
        if (!clashesKeptAnchor(p)) landmarkKept.push(p);
    }
    landmarkLayer = L.layerGroup(
        landmarkKept.map(p => anchorMarker(L, p, 'map-landmark-wrap', 'map-landmark-label'))
    ).addTo(map);
}

// landmarks.json spots（策展夜市/廟宇/景點 + OSM 百貨）併入地標層：
// 「士林夜市、龍山寺」比商圈名更會認。商圈先建先贏，同點/同名不重標。
function addLandmarkSpots(L, spots) {
    if (!landmarkLayer || !map) return;
    for (const p of spots) {
        if (!Number.isFinite(p.lat) || !Number.isFinite(p.lng)) continue;
        if (clashesKeptAnchor(p)) continue;
        landmarkKept.push(p);
        landmarkLayer.addLayer(anchorMarker(L, p, 'map-landmark-wrap', 'map-landmark-label'));
    }
}

// ---- 捷運站錨點（r60）----
// 為什麼獨立一層、不併進上面的地標層：地標層的「捷運站名」是從餐廳的地標 tag 反推、
// 座標取附近餐廳的重心（見 generate_map_pins.py 的 landmark_pts），位置不準也不完整，
// 而且跟商圈名同色同字級，看不出哪個是車站；更關鍵的是地標層 z≥17 就讓位給街道字——
// 但使用者要對方位時往往正在放大。本層改用 OSM 真實站點座標（fetch_mrt_stations.py），
// 站名前面標官方路線色圓點（轉乘站多顆），z≥14 起一路顯示到最大不消失。
...
function mrtDotsHtml(lines) {
    return (lines || [])
        .map(l => `<i class="map-mrt-dot" style="background:${MRT_LINE_COLORS[l] || '#8d6e63'}"></i>`)
        .join('');
}
...
function buildExtLayer(L) {
    ...
    for (const poi of extPois) {
        const m = L.circleMarker([poi.lat, poi.lng], {
            ...
            weight: 1.2,
            fillColor: '#FFFFFF',
            ...DOT_MUTED, // r40 降噪：z16 半透明底紋，z≥17 恢復實心（見 syncPinSize）
...
function showMiniCard(pin, source = 'unknown') {
    ...
    // 版面（2026-07 改版）：Google 式「小頭圖＋全寬內容」——
...
export async function initMapPage() {
    ...
    try {
        ...
        // 分店去重需要 extPois 先就位。同為選配資料，失敗地圖照常。
        const loadLandmarks = () => fetch(new URL('../data/landmarks.json', import.meta.url))
            .then(r => (r.ok ? r.json() : null))
            .then(res => {
                if (!res || !map) return;
                addLandmarkSpots(L, res.spots || []);
                buildBrandLayer(L, res.brands || []);
            })
            .catch(() => { /* 選配資料，靜默失敗 */ });
        // 捷運站錨點（r60）：檔案小（~10 KB）且是「這是哪裡」的第一線索，排在延後鏈最前面。
        ...
        // r40 開場節制：一次只說一句——今天第一次開才報連續天數（同日重開不再喊），
        // 首次使用讓位給下方的導覽提示（showPillMessage 單一插槽，晚到會蓋掉早到）。
        const firstBootToday = getStreak().last !== localDateKey();
        ...
        const syncLabels = () => {
            ...
            // 地標錨點：z14–16 當空間座標；z≥17 街道/店名接手、z<14 太密都收掉
            el.classList.toggle('show-landmarks', z >= 14 && z < 17);
            // 連鎖品牌招牌（麥當勞…）：z≥16 街區層級才出（分店密，太早出會蓋掉商圈錨點）
            el.classList.toggle('show-brands', z >= 16);
            // 捷運站（r60）：z≥14 起就出，且刻意「不設上限」——放大時才最需要對方位，
            // 商圈錨點 z≥17 讓位給街道字是對的，但車站讓位就沒東西可定位了（用戶回報）。
            el.classList.toggle('show-mrt', z >= 14);
        ...
        // 拉近時灰點放大且恢復實心（手指點得到、街道層級灰點就是內容）；
        // 拉遠退回半透明小點（r40 降噪：不跟彩色優惠釘搶注意力）
        let pinBumped = false;
        const syncPinSize = () => {
            const bump = map.getZoom() >= 17;
            if (bump === pinBumped) return;
            pinBumped = bump;
            const style = bump ? DOT_SOLID : DOT_MUTED;
            // 只有灰點(circleMarker)有 setRadius；餐廳 icon(divIcon marker)固定尺寸，跳過。
            pinMarkers.forEach(m => {
                if (m.setRadius) m.setStyle({ radius: m._baseRadius + (bump ? 2 : 0), ...style });
            });
            if (extLayer) extLayer.eachLayer(m => m.setStyle({ radius: bump ? 6 : 4, ...style }));
        };
