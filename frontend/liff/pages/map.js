...
let landmarkLayer = null;       // 地標錨點標籤層（商圈/捷運站名，r40：低 zoom 的空間錨點）
...
// ---- 地標錨點（r40）：商圈/捷運站名直接標在地圖上 ----
// Google 一載入就有「行天宮、南京復興」等地名當空間錨點，我們的底圖磚在中低 zoom
// 幾乎沒字 → 用戶不知道自己在看哪。從搜尋索引的地標（places, t=landmark）挑熱門者
// 畫成純文字標籤：z14–16 顯示（CSS .show-landmarks 控制），z≥17 街道字接手就讓位。
function buildLandmarkLayer(L) {
    ...
    if (!cands.length || !map) return;
    // 同名異寫的地標常幾乎同點（中山 vs 中山捷運線型商圈）：留店數多的，300m 內去重
    const kept = [];
    for (const p of cands) {
        if (kept.length >= 28) break; // 錨點是配角，鋪滿反而變雜訊
        const clash = kept.some(k =>
            Math.abs(k.lat - p.lat) < 0.0027 && Math.abs(k.lng - p.lng) < 0.003); // ≈300m
        if (!clash) kept.push(p);
    }
    landmarkLayer = L.layerGroup(kept.map(p => L.marker([p.lat, p.lng], {
        icon: L.divIcon({
            className: 'map-landmark-wrap',
            html: `<span class="map-landmark-label">${escapeHtml(p.n)}</span>`,
            iconSize: [0, 0],
        }),
